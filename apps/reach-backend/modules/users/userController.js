...
async function registerUser(req, res) {
  ...
  try {
    ...
    console.log(
      "Error has occured in register route: ",
      e?.stack || e?.message || e,
    );
...
const handleFriendRequest = async (req, res) => {
  ...
  try {
    const [userDetails = {}, callerDetails = {}] =
      (await Promise.all([
    ...
    const {
      friend_requests: callerFriendRequests = [],
      friend_list: callerFriendList = [],
    } = callerDetails || {};
...
const getPendingFriendRequests = async (req, res) => {
  ...
  try {
    ...
    const pendingRequests =
      (await User.find({ userId: { $in: friend_requests } }).lean()) || [];
    res.status(200).send({ status: "success", data: pendingRequests });
  } catch (e) {
    console.log(
      "An Error has occured in the getPendingFriendRequests route: ",
      e,
    );
    errorHandler(req, res, e);
  }
...
const getFriendList = async (req, res) => {
  ...
  try {
    ...
    const friendList =
      (await User.find({ userId: { $in: friend_list } }).lean()) || [];
